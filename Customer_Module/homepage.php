...
<section class="features" id="features">
    ...
    <div class="features-grid">
        ...
        <div class="feature-card"><div class="feature-icon"><i class="fas fa-clock"></i></div><h3>Extended Hours</h3><p>Open daily 8am - 1am. Early bird and late night sessions available for night owls.</p></div>
...
<footer class="footer">
    <div class="footer-container">
        ...
        <div class="footer-col"><h4>Quick Links</h4><a href="dashboard.php">Find a Court</a><a href="my_bookings.php">My Booking</a><a href="../Payment_Module/wallet.php">Wallet</a></div>
...
<div id="loginPasswordMode"><input type="email" id="loginEmail" placeholder="Email address"><input type="password" id="loginPassword" placeholder="Password"><button class="btn-primary-modal" id="doPasswordLogin">Login with Password</button></div>
<div class="hr-text">———— OR ————</div>
<div id="loginOtpMode"><input type="email" id="loginOtpEmail" placeholder="Email address"><button class="btn-secondary-modal" id="sendLoginOtpBtn">Send OTP Code</button><input type="text" id="loginOtpCode" placeholder="Enter 6-digit OTP" style="display:none;"><button class="btn-primary-modal" id="verifyLoginOtpBtn" style="display:none;">Verify & Login</button></div>
...
<div id="forgotStep1"><label>Email Address</label><input type="email" id="forgotEmail" placeholder="Enter your registered email"><button class="btn-primary-modal" id="sendResetOtpBtn">Send Reset Code</button></div>
<div id="forgotStep2" style="display:none;"><label>Verification Code</label><input type="text" id="forgotOtpCode" placeholder="Enter 6-digit code"><label>New Password</label><input type="password" id="forgotNewPassword" placeholder="At least 6 characters + 1 symbol"><div class="strength-meter"><div class="strength-meter-fill" id="forgotStrengthFill"></div></div><div id="forgotStrengthText" class="strength-text"></div><label>Confirm Password</label><input type="password" id="forgotConfirmPassword" placeholder="Confirm your new password"><div id="forgotPasswordMatch" class="password-match"></div><button class="btn-primary-modal" id="resetPasswordBtn">Reset Password</button></div>
...
<script>
    ...
    function validatePasswordMatch() { const password = regPasswordInput.value; const confirm = regConfirmPassword.value; if(confirm.length === 0) { passwordMatchDiv.innerHTML = ''; passwordMatchValid = false; } else if(password === confirm) { passwordMatchDiv.innerHTML = '<span class="valid">✓ Passwords match</span>'; passwordMatchValid = true; } else { passwordMatchDiv.innerHTML = '<span class="invalid">✗ Passwords do not match</span>'; passwordMatchValid = false; } validateRegisterButton(); }
    ...
    let nameValid = false;
    regNameInput.addEventListener('blur', async function() { const name = this.value.trim(); if(name.length < 2) { nameStatusDiv.innerHTML = '<span class="username-invalid">Name must be at least 2 characters</span>'; nameValid = false; return; } try { const res = await fetch(baseUrl + 'check_username.php?name=' + encodeURIComponent(name)); const data = await res.json(); if(data.exists) { nameStatusDiv.innerHTML = '<span class="username-invalid">❌ This name is already taken</span>'; nameValid = false; } else { nameStatusDiv.innerHTML = '<span class="username-valid">✓ Name available</span>'; nameValid = true; } } catch(e) { nameStatusDiv.innerHTML = '<span class="username-invalid">Error checking name</span>'; nameValid = false; } validateRegisterButton(); });

    const strengthFill = document.getElementById('passwordStrengthFill');
    const strengthText = document.getElementById('passwordStrengthText');
    let passwordValid = false;
    function checkPasswordStrength(pwd) { let score = 0; if(pwd.length >= 6) score++; if(pwd.length >= 8) score++; if(/[!@#$%^&*(),.?":{}|<>]/.test(pwd)) score++; if(/[A-Z]/.test(pwd) && /[a-z]/.test(pwd)) score++; if(/\d/.test(pwd)) score++; if(pwd.length === 0) return { percent: 0, text: '', valid: false }; let percent = 0, text = '', valid = false; if(score <= 2) { percent = 25; text = 'Weak'; valid = false; } else if(score === 3) { percent = 50; text = 'Fair'; valid = false; } else if(score === 4) { percent = 75; text = 'Good'; valid = true; } else { percent = 100; text = 'Strong'; valid = true; } const hasSymbol = /[!@#$%^&*(),.?":{}|<>]/.test(pwd); if(pwd.length >= 6 && hasSymbol && score >= 3) valid = true; else valid = false; if(pwd.length < 6) { text = 'Too short (min 6)'; valid = false; } else if(!hasSymbol) { text = 'Need at least 1 symbol (!@#$...)'; valid = false; } return { percent, text, valid }; }
    regPasswordInput.addEventListener('input', function() { const pwd = this.value; const result = checkPasswordStrength(pwd); strengthFill.style.width = result.percent + '%'; if(result.percent <= 25) strengthFill.style.background = '#e67e22'; else if(result.percent <= 50) strengthFill.style.background = '#f1c40f'; else if(result.percent <= 75) strengthFill.style.background = '#2b7e3a'; else strengthFill.style.background = '#2b7e3a'; strengthText.innerText = result.text; passwordValid = result.valid; validateRegisterButton(); validatePasswordMatch(); });
    ...
    if(forgotNewPassword) { forgotNewPassword.addEventListener('input', function() { const pwd = this.value; const result = checkPasswordStrength(pwd); forgotStrengthFill.style.width = result.percent + '%'; if(result.percent <= 25) forgotStrengthFill.style.background = '#e67e22'; else if(result.percent <= 50) forgotStrengthFill.style.background = '#f1c40f'; else if(result.percent <= 75) forgotStrengthFill.style.background = '#2b7e3a'; else forgotStrengthFill.style.background = '#2b7e3a'; forgotStrengthText.innerText = result.text; if(forgotConfirmPassword.value.length > 0) { if(pwd === forgotConfirmPassword.value) { forgotPasswordMatchDiv.innerHTML = '<span class="valid">✓ Passwords match</span>'; } else { forgotPasswordMatchDiv.innerHTML = '<span class="invalid">✗ Passwords do not match</span>'; } } }); forgotConfirmPassword.addEventListener('input', function() { if(this.value.length > 0 && this.value === forgotNewPassword.value) { forgotPasswordMatchDiv.innerHTML = '<span class="valid">✓ Passwords match</span>'; } else if(this.value.length > 0) { forgotPasswordMatchDiv.innerHTML = '<span class="invalid">✗ Passwords do not match</span>'; } else { forgotPasswordMatchDiv.innerHTML = ''; } }); }

    function validateRegisterButton() { const nameOk = nameValid; const emailOk = regEmail.value.trim().includes('@'); const phoneOk = regPhone.value.trim().length > 5; const otpVerified = regVerified; const passwordOk = passwordValid && passwordMatchValid; const registerBtn = document.getElementById('registerFinalBtn'); if(nameOk && emailOk && phoneOk && passwordOk && otpVerified) { registerBtn.disabled = false; } else { registerBtn.disabled = true; } }
    ...
    sendRegCodeBtn.onclick = async () => { const name = regName.value.trim(); const email = regEmail.value.trim(); const password = regPassword.value; const phoneFull = regPhoneCode.value + regPhoneInput.value.trim(); if(!nameValid) { showError(regError, "Please choose a valid unique name."); return; } if(!passwordValid) { showError(regError, "Password must be at least 6 characters and contain at least one symbol (!@#$...)."); return; } if(!name || !email || !password || !regPhoneInput.value.trim()) { showError(regError, "Please fill all fields."); return; } setButtonLoading(sendRegCodeBtn, true); try { const res = await fetch(baseUrl+'send_otp.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ email: email, type:'register' }) }); const data = await res.json(); if(data.success) { regEmailStored = email; regVerified = false; showError(regError, "✓ OTP sent to your email. Check your inbox!"); regVerifyCode.style.display = 'block'; verifyRegCodeBtn.style.display = 'block'; } else { showError(regError, data.message); } } catch(err) { showError(regError, "Network error: " + err.message); } finally { setButtonLoading(sendRegCodeBtn, false); } };
    
    verifyRegCodeBtn.onclick = async () => { const code = regVerifyCode.value.trim(); const email = regEmailStored; if(!email || !code) { showError(regError, "Enter the code."); return; } setButtonLoading(verifyRegCodeBtn, true); try { const res = await fetch(baseUrl+'verify_otp.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ email, code, type:'register' }) }); const data = await res.json(); if(data.success) { regVerified = true; showError(regError, "✓ Verified! You can now register."); validateRegisterButton(); } else { showError(regError, data.message); } } catch(err) { showError(regError, "Verification failed."); } finally { setButtonLoading(verifyRegCodeBtn, false); } };
    
    registerFinalBtn.onclick = async () => { if(!regVerified) { showError(regError, "Verify your email first."); return; } if(!nameValid) { showError(regError, "Name is invalid or already taken."); return; } if(!passwordValid || !passwordMatchValid) { showError(regError, "Password does not meet requirements or passwords do not match."); return; } const name = regName.value.trim(); const email = regEmail.value.trim(); const password = regPassword.value; const phoneFull = regPhoneCode.value + regPhoneInput.value.trim(); registerFinalBtn.disabled = true; registerFinalBtn.innerText = "Registering..."; try { const res = await fetch(baseUrl+'register.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ name, email, password, phone:phoneFull }) }); const data = await res.json(); if(data.success) { alert("Registration successful! Please login."); closeAll(); regName.value = ''; regEmail.value = ''; regPassword.value = ''; regConfirmPassword.value = ''; regPhoneInput.value = ''; regVerifyCode.value = ''; registerFinalBtn.disabled = true; regVerified = false; nameValid = false; passwordValid = false; passwordMatchValid = false; } else { showError(regError, data.message); } } catch(err) { showError(regError, "Registration failed."); } finally { registerFinalBtn.disabled = false; registerFinalBtn.innerText = "Register"; } };

    const loginEmail = document.getElementById('loginEmail');
    const loginPassword = document.getElementById('loginPassword');
    const doPasswordLogin = document.getElementById('doPasswordLogin');
    const loginError = document.getElementById('loginError');
    doPasswordLogin.onclick = async () => { const email = loginEmail.value.trim(); const password = loginPassword.value; if(!email || !password) { showError(loginError, "Enter email and password."); return; } doPasswordLogin.disabled = true; doPasswordLogin.innerText = "Logging in..."; try { const res = await fetch(baseUrl+'login_password.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ email, password }) }); const data = await res.json(); if(data.success) { window.location.href = baseUrl+'dashboard.php'; } else { showError(loginError, data.message); } } catch(err) { showError(loginError, "Login failed."); } finally { doPasswordLogin.disabled = false; doPasswordLogin.innerText = "Login with Password"; } };
    ...
    sendLoginOtpBtn.onclick = async () => { const email = loginOtpEmail.value.trim(); if(!email) { showError(loginError, "Enter email."); return; } setButtonLoading(sendLoginOtpBtn, true); try { const res = await fetch(baseUrl+'send_otp.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ email, type:'login' }) }); const data = await res.json(); if(data.success) { loginOtpCode.style.display = 'block'; verifyLoginOtpBtn.style.display = 'block'; showError(loginError, "✓ OTP sent to your email. Check your inbox!"); } else { showError(loginError, data.message); } } catch(err) { showError(loginError, "Failed to send OTP."); } finally { setButtonLoading(sendLoginOtpBtn, false); } };
    verifyLoginOtpBtn.onclick = async () => { const email = loginOtpEmail.value.trim(); const code = loginOtpCode.value.trim(); if(!email || !code) { showError(loginError, "Enter OTP."); return; } setButtonLoading(verifyLoginOtpBtn, true); try { const verifyRes = await fetch(baseUrl+'verify_otp.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ email, code, type:'login' }) }); const verifyData = await verifyRes.json(); if(!verifyData.success) { showError(loginError, verifyData.message); return; } const loginRes = await fetch(baseUrl+'login_otp.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ email }) }); const loginData = await loginRes.json(); if(loginData.success) { window.location.href = baseUrl+'dashboard.php'; } else { showError(loginError, loginData.message); } } catch(err) { showError(loginError, "OTP login failed."); } finally { setButtonLoading(verifyLoginOtpBtn, false); } };
    
    const sendResetOtpBtn = document.getElementById('sendResetOtpBtn');
    if(sendResetOtpBtn) { sendResetOtpBtn.onclick = async () => { const email = document.getElementById('forgotEmail').value.trim(); if(!email) { showError(document.getElementById('forgotError'), "Please enter your email"); return; } setButtonLoading(sendResetOtpBtn, true); try { const res = await fetch(baseUrl+'send_reset_otp.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email }) }); const data = await res.json(); if(data.success) { document.getElementById('forgotStep1').style.display = 'none'; document.getElementById('forgotStep2').style.display = 'block'; showSuccess(document.getElementById('forgotSuccess'), "Reset code sent to your email!"); } else { showError(document.getElementById('forgotError'), data.message); } } catch(err) { showError(document.getElementById('forgotError'), "Network error"); } finally { setButtonLoading(sendResetOtpBtn, false); } }; }
    
    const resetPasswordBtn = document.getElementById('resetPasswordBtn');
    if(resetPasswordBtn) { resetPasswordBtn.onclick = async () => { const email = document.getElementById('forgotEmail').value.trim(); const code = document.getElementById('forgotOtpCode').value.trim(); const newPassword = document.getElementById('forgotNewPassword').value; const confirmPassword = document.getElementById('forgotConfirmPassword').value; if(!code) { showError(document.getElementById('forgotError'), "Please enter verification code"); return; } if(newPassword !== confirmPassword) { showError(document.getElementById('forgotError'), "Passwords do not match"); return; } const pwdResult = checkPasswordStrength(newPassword); if(!pwdResult.valid) { showError(document.getElementById('forgotError'), pwdResult.text); return; } setButtonLoading(resetPasswordBtn, true); try { const res = await fetch(baseUrl+'reset_password.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, code, new_password: newPassword }) }); const data = await res.json(); if(data.success) { showSuccess(document.getElementById('forgotSuccess'), "Password reset successful! Please login."); setTimeout(() => { forgotPasswordModal.style.display = 'none'; openLogin(); }, 2000); } else { showError(document.getElementById('forgotError'), data.message); } } catch(err) { showError(document.getElementById('forgotError'), "Network error"); } finally { setButtonLoading(resetPasswordBtn, false); } }; }
</script>
</body>
</html>