...
<head>
    ...
    <style>
        /* 所有样式保持不变（与用户提供的原样式相同，此处省略以节省篇幅，实际使用时请保留完整样式） */
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif; }
        ...
        .modal-content { background-color:white; margin:5% auto; padding:2rem; width:90%; max-width:450px; border-radius:28px; box-shadow:0 20px 35px rgba(0,0,0,0.2); position:relative; animation:fadeInUp 0.3s ease; }
        @keyframes fadeInUp { from { opacity:0; transform:translateY(30px);} to { opacity:1; transform:translateY(0);} }
        .close { position:absolute; right:1.5rem; top:1rem; font-size:1.8rem; cursor:pointer; color:#aaa; }
        .close:hover { color:#0099ff; }
        .modal-content h2 { color:#0099ff; margin-bottom:1.5rem; text-align:center; }
        .modal-content input, .modal-content select { width:100%; padding:0.8rem; margin:0.5rem 0 1rem; border:1px solid #ccc; border-radius:40px; font-size:1rem; outline:none; }
        .modal-content input:focus { border-color:#0099ff; }
        ...
        .btn-secondary-modal { background:white; border:1px solid #0099ff; color:#0099ff; }
        .hr-text { text-align:center; margin:1rem 0; color:#aaa; }
        .toggle-link { text-align:center; margin-top:1rem; font-size:0.9rem; }
        .toggle-link a { color:#0099ff; text-decoration:none; cursor:pointer; }
        .error-msg { color:#e74c3c; font-size:0.8rem; margin-top:-0.5rem; margin-bottom:0.5rem; }
        ...
        @media (max-width:768px) { .hero { flex-direction:column; text-align:center; } .nav-links { gap:1rem; } .btn-outline,.btn-solid { padding:0.3rem 0.8rem; } .footer-grid { grid-template-columns:1fr; text-align:center; } .social-icons { justify-content:center; } }
    </style>
</head>
<body>
...
<script>
    ...
    sendRegCodeBtn.onclick = async () => {
        ...
        if (!name || !email || !password || !nric || !regPhone.value.trim()) {
    ...
    registerFinalBtn.onclick = async () => {
        ...
        if (data.success) {
            ...
            closeAllModals();
            // 重置表单
            regName.value = ''; regEmail.value = ''; regPassword.value = ''; regNric.value = ''; regPhone.value = '';
