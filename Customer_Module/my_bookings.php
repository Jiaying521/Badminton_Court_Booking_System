...
<script>
    ...
    function openRescheduleModal(bookingId, courtId, date, time, hours, rescheduleCount) {
        if (rescheduleCount >= 1) {
            alert('⚠️ This booking has already been rescheduled.Each booking can only be rescheduled once.Please make a new booking if you need a different time.');
            return;
        }
    ...
    async function cancelBooking(bookingId, currentCancellationCount) {
        try {
            ...
            if(data.success) {
                ...
                if (hoursDiff >= 48) {
                    if (isSecondOrMore) {
                        confirmMessage = `🏸 CANCELLATION POLICY (≥48 hours notice) - 2nd Cancellation\n\n` +
                            `Booking: ${booking.court_name}\n` +
                            `Date: ${booking.booking_date}\n` +
                            `Time: ${booking.start_time} - ${booking.end_time}\n\n` +
                            `⚠️ This is your 2nd cancellation!\n` +
                            `📌 Additional penalty of RM 5.00 applies\n` +
                            `💰 Refund: RM ${(parseFloat(booking.total_price) - 5).toFixed(2)}\n\n` +
                    ...
                    if (hasCoach) {
                        if (isSecondOrMore) {
                            confirmMessage = `🏸 CANCELLATION POLICY (24-48 hours notice) - Training Mode (2nd Cancellation)\n\n` +
                                `Booking: ${booking.court_name}\n` +
                                `Date: ${booking.booking_date}\n` +
                                `Time: ${booking.start_time} - ${booking.end_time}\n\n` +
                                `⚠️ This is your 2nd cancellation!\n` +
                        ...
                        if (isSecondOrMore) {
                            const refundAmount = parseFloat(booking.total_price) - 10 - 5;
                            confirmMessage = `🏸 CANCELLATION POLICY (24-48 hours notice) - Play Only (2nd Cancellation)\n\n` +
                                `Booking: ${booking.court_name}\n` +
                                `Date: ${booking.booking_date}\n` +
                                `Time: ${booking.start_time} - ${booking.end_time}\n\n` +
                                `⚠️ This is your 2nd cancellation!\n` +
                                ...
                                `📌 Additional penalty of RM 5.00 applies\n` +
                                `💰 Refund: RM ${refundAmount.toFixed(2)}\n\n` +
                                `Do you want to proceed with cancellation?`;
                        } else {
